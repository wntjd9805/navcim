    ...
    if (tmp_dfg[str(i)][0]=='nn.conv2d' or tmp_dfg[str(i)][0]=='nn.dense'):
      ...
      if dfg_key[-1] == i:
        ...
        if args.search_accuracy == 1:
          total_similarity1 = total_similarity + (Similarity_for_accuracy[f"{NUM1_ROW},{NUM1_COL}"][conv_dense-1]* math.pow(hessian_list[conv_dense-1],2))
...
if args.search_accuracy == 1:
  #raise error
  df = pd.read_csv(f'{navcim_dir}/cross-sim/applications/dnn/inference/{args.model}_{args.search_accuracy_metric}_list.csv')
  Similarity_for_accuracy = {col: df[col].tolist() for col in df.columns}
...
# print(CONFIG_pareto)
# print("CONFIG_pareto", len(CONFIG_pareto))
grouped_configs={}
...
for group in grouped_configs.values():
    # 현재 그룹의 크기가 args.heterogeneity 이상인 경우에만 조합을 계산
    print(len(group))
    if len(group) >= args.heterogeneity:
        combine_CONFIG.extend(list(combinations(group, args.heterogeneity)))

# combine_CONFIG = list(combinations(CONFIG_pareto,args.heterogeneity))
...
def update_progress(result):
    """작업 완료 시 호출되어 진행 상황을 업데이트하는 콜백 함수."""
    with lock_w:
...
if not os.path.isdir(f"{navcim_dir}/Inference_pytorch/search_result/{args.model}_hetero"):
  os.makedirs(f"{navcim_dir}/Inference_pytorch/search_result/{args.model}_hetero")

if args.search_accuracy == 1:
  final_latency_f = open(f"./search_result/{args.model}_hetero/final_result_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area},{args.accuracy}]_{args.heterogeneity}_{args.search_accuracy_metric}.txt",'w', newline='')
else:
  final_latency_f = open(f"./search_result/{args.model}_hetero/final_result_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area}]_{args.heterogeneity}.txt",'w', newline='')
...
if args.search_accuracy == 1:
  final_log_f = open(f"./search_result/{args.model}_hetero/final_log_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area},{args.accuracy}]_{args.heterogeneity}_{args.search_accuracy_metric}.txt",'w', newline='')
else:
  final_log_f = open(f"./search_result/{args.model}_hetero/final_log_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area}]_{args.heterogeneity}.txt",'w', newline='')
