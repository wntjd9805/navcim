...
parser.add_argument('--models', default='ResNet50', help='for open file')
...
if args.search_accuracy == 1:
  open_file = f"./model_search_result_architechture_{args.models}_hetero_with_accuracy.txt"
else:
  open_file = f"./model_search_result_architechture_{args.models}_hetero.txt"

with open(open_file) as f:
  lines = f.readline()
  split_config = lines.split(":")
  split_config = split_config[:-1]

  for input_string in split_config:
    string_to_list = eval(input_string)
    combine_CONFIG.append(string_to_list)
    if string_to_list[0][4] not in adc_set: 
      adc_set.append(string_to_list[0][4]) 
    if string_to_list[0][5] not in cellbit_set:
      cellbit_set.append(string_to_list[0][5])
    for i in range(args.heterogeneity):
      if string_to_list[i] not in CONFIG:
        CONFIG.append(string_to_list[i])
...
def inverse_minmax_energy(value, max, min):
    #max_, min_ 값 찾아야함
    newmax = 10
    newmin = 0
    max_ = max
...
def number_of_tile_sofar(mapping_info):
  ...
  num_chip2=0
  # print(mapping_info)
  ## 여기 코드도 heterogeneity에 따라 다르게 바꿔야 함
...
def predict_booksim(node, cluster_width, chip1_width, cluster_flit_cycle, chip1_flit_cycle ,model, select, cluster_meter, chip1_meter , list_for_predict, need_area_leak):  
  ...
  # hop_average = calculate_average_hops(list_for_predict["num_of_input_cluster"], list_for_predict["num_of_dest_cluster"], (cluster_width,cluster_width))
  ...
  with torch.no_grad():
    ...
    # latency_predict_cluster = scaler_y_latency.inverse_transform(latency_predict_normalized)[0][0]
    ...
    # power_predict_cluster = np.expm1(inverse_minmax_latency(energy_predict_normalized,scaler_y_params_energy['max'], scaler_y_params_energy['min'])[0][0])
  ...
  # energy_result = (power_predict_cluster * latency_predict_cluster * 1000) + (power_predict_element_send * latency_predict_element_send * 1000) + (power_predict_element_receive * latency_predict_element_receive * 1000)
  power_result = power_predict_cluster 
  power_result_cluster = power_predict_element_send + power_predict_element_receive 
  # print(power_predict_cluster, input_data_cluster)
  # print(latency_result, energy_result)
  if need_area_leak == 1:
  ...
  for i in dfg_key:
    ...
    if (tmp_dfg[str(i)][0]=='nn.conv2d' or tmp_dfg[str(i)][0]=='nn.dense'):
      ...
      if depth==0: #or depth ==1:
        ...
        # os.makedirs(f"./record_predict_{args.model}/Prepared_data/{args.heterogeneity}/{depth}/{NUM_ROW_cluster}/{NUM_COL_cluster}", exist_ok=True)
        # if os.path.isfile(f"./record_predict_{args.model}/Prepared_data/{args.heterogeneity}/{depth}/{NUM_ROW_cluster}/{NUM_COL_cluster}/CLUSTER_{args.model}_{file_name}{tail1}{i}.txt"):
        #   fname = f"./record_predict_{args.model}/Prepared_data/{args.heterogeneity}/{depth}/{NUM_ROW_cluster}/{NUM_COL_cluster}/CLUSTER_{args.model}_{file_name}{tail1}{i}.txt"
        #   try:
        #     with open(fname) as f:
        #       lines = f.readlines()
        ...
        #       prepared1 = open(f"./record_predict_{args.model}/Prepared_data/{args.heterogeneity}/{depth}/{NUM_ROW_cluster}/{NUM_COL_cluster}/CLUSTER_{args.model}_{file_name}{tail1}{i}.txt",'w')
        ...
        # print(fname)
        # print("bosoek", total_area , cluster_booksim_area , chip1_booksim_area, cluster_area, addition_cluster)
      ...
      if dfg_key[-1] == i:
        ...
        # mlp_latency = scaler_x_scaler_latency.transform([[booksim_latency1, compute_PPA1[conv_dense][0]]])
        # scale_latency = loaded_scaler_latency.predict(mlp_latency)
        # scale_latency = scaler_y_scaler_latency.inverse_transform(scale_latency)
        
        booksim_energy = booksim_energy1 * (scale_latency[0][0]-compute_PPA1[conv_dense][0]) * 1000
        # booksim_leakage_energy = chip1_booksim_leakage * (scale_latency[0][0]-compute_PPA1[conv_dense][0]) * 1000
        # poly_energy = polynomial_features_energy.transform([[booksim_energy, compute_PPA1[conv_dense][1]], booksim_leakage_energy])
        # scale_energy = linear_model_energy.predict(poly_energy)
      ...
      #   # else:
      #   booksim_latency_non_mac,booksim_energy_non_mac,cluster_booksim_area_non_mac,chip1_booksim_area_non_mac,cluster_booksim_leakage_non_mac,chip1_booksim_leakage_non_mac  = execute_booksim(i,chip_width,int(CLUSTER1),cluster_flit_cycle,chip1_flit_cycle,args.model,tail1,cluster_meter,chip1_meter,chip1_period,period,cluster_buswidth,chip1_buswidth,file_name, NUM_ROW_cluster, NUM_COL_cluster)
      #   predict_booksim(i,chip_width,int(CLUSTER1),cluster_flit_cycle,chip1_flit_cycle,args.model,tail1,cluster_meter,chip1_meter, list_for_predict)
      #     # if booksim_latency_non_mac >0 :
      #     #   prepared1 = open(f"./record_predict_{args.model}/Prepared_data/{args.heterogeneity}/{depth}/{NUM_ROW_cluster}/{NUM_COL_cluster}/CLUSTER_{args.model}_{file_name}{tail1}{i}.txt",'w')
      #     #   wr_prepared1 = csv.writer(prepared1)
      #     #   wr_prepared1.writerow([booksim_latency_non_mac,booksim_energy_non_mac])
      ...
      # print(f"k1:{key_position1},k2:{key_position2},i:{i},input_cluster:{input_cluster},mapping_num_list:{mapping_num_list1}")
      list_for_predict={}
      ...
      #booksim_latency_non_mac_cluster,booksim_energy_non_mac_cluster = 0
      booksim_latency_non_mac,booksim_power_non_mac, booksim_latency_non_mac_cluster,booksim_energy_non_mac_cluster = predict_booksim(i,chip_width,int(CLUSTER1),cluster_flit_cycle,chip1_flit_cycle,args.model,tail1,cluster_meter,chip1_meter, list_for_predict,0)
       ...
       #여기 부터가 그거 neurosim 이랑 합치는 부분이네 그렇지 않을까?
      ...
      if depth == 0 :
        ...
        # mlp_latency = scaler_x_scaler_latency.transform([[before_conv_result1[0][0],before_conv_result1[0][1]]])
        # scale_latency = loaded_scaler_latency.predict(mlp_latency)
        # scale_latency = scaler_y_scaler_latency.inverse_transform(scale_latency)
        # print(before_conv_result1[1][1])
        # print(booksim_power_non_mac)  
        # booksim_leakage_energy = chip1_booksim_leakage * (scale_latency[0][0]-before_conv_result1[0][1]) * 1000
        # print(before_conv_result1[1][0]* (scale_latency[0][0]-before_conv_result1[0][1]) * 1000)
        # poly_energy = polynomial_features_energy.transform([[before_conv_result1[1][0]*(scale_latency[0][0]-before_conv_result1[0][1])*1000,before_conv_result1[1][1], booksim_leakage_energy]])
        # scale_energy = linear_model_energy.predict(poly_energy)
        ...
        # print("latency predict: ",scale_latency[0][0]-before_conv_result1[0][1])
        # print("energy predict: ",scale_energy[0] - before_conv_result1[1][1])
      ...
      selected_list1.append(ht)
      # print("total latency: ",total_latency1, "total energy: ",total_energy1)
      # print(total_leakage1, total_area1)
      # print("juseong",selected_list1,total_leakage1)
      if args.search_accuracy == 1:
        total_similarity1 = total_similarity + (Similarity_for_accuracy[f"{ADC},{Cellbit}"][f"{NUM1_ROW},{NUM1_COL}"][conv_dense-1] * math.pow(hessian_list[conv_dense-1],2))
        return iter1,level1,count1,mapping_num_list1,tile_grid1,mapping_info1,conv_dense,float(total_latency1),float(total_energy1),total_area1,total_leakage1,selected_list1,Booksim_Leakage_Area,total_similarity1
      else:
...
# CONFIG = [[128,128,2,8],[128,128,4,8]]  
print("STRAT")
for config in CONFIG:
  NUM_ROW = config[0]
  NUM_COL = config[1]
...
for exec_set in execute_set:
  ...
  # start = time.time()
  # for i in range(len(combine_CONFIG)):
  #   my_func(exec_set_list_repeated[i])
  #   if i % 100 == 0:
  #     print(f"{i}/{len(combine_CONFIG)} tasks completed.")
  end =time.time()
...
if not os.path.isdir(f"{navcim_dir}/Inference_pytorch/search_result/{args.models}_multi_model_step3"):
  os.makedirs(f"{navcim_dir}/Inference_pytorch/search_result/{args.models}_multi_model_step3")

if args.search_accuracy == 1:
  final_latency_f = open(f"./search_result/{args.models}_multi_model_step3/final_result_{args.model}_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area},{args.accuracy}]_{args.heterogeneity}_{args.search_accuracy_metric}.txt",'w', newline='')
else:
  final_latency_f = open(f"./search_result/{args.models}_multi_model_step3/final_result_{args.model}_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area}]_{args.heterogeneity}_pareto.txt",'w', newline='')
...
if args.search_accuracy == 1:
  final_log_f = open(f"./search_result/{args.models}_multi_model_step3/final_log_{args.model}_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area},{args.accuracy}]_{args.heterogeneity}_{args.search_accuracy_metric}.txt",'w', newline='')
else:
  final_log_f = open(f"./search_result/{args.models}_multi_model_step3/final_log_{args.model}_{args.beam_size_m}_{args.beam_size_n}_{args.distribute}_[{args.latency},{args.power},{args.area}]_{args.heterogeneity}_pareto.txt",'w', newline='')
