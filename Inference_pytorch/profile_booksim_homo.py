...
def execute_booksim(node, cluster_width, chip1_width, chip2_width, cluster_flit_cycle, chip1_flit_cycle, chip2_flit_cycle,model,chip1, cluster_meter, chip1_meter,chip2_meter ,chip1_buswidth,period):
  cmd1 = f'../booksim2/src/booksim ../booksim2/src/examples/mesh88_lat {cluster_width} {chip1_width} {cluster_flit_cycle} {chip1_flit_cycle} ../Inference_pytorch/record_{args.model}/homo/CLUSTER_{model}_{chip1}.txt ../Inference_pytorch/record_{args.model}/homo/SMALL_{model}_{chip1}.txt {cluster_meter} {chip1_meter} {chip1_buswidth} {chip1_buswidth} 0 na {node} 1 | egrep "taken|Total Power|Total Area|Total leak Power" > ./record_{args.model}/homo/BOOKSIM_{model}_{chip1}.txt'
...
def make_args(config):
  ...
  compute_PPA1={}
  node_col={}
  fname1 = f"./shape/shape_{args.model}_ADC:5_Cellbit:2_SA_row:{NUM1_ROW}_SA_col:{NUM1_COL}_PE:{PE1}_TL:{Tile1}"
...
for config in CONFIG:
  ...
  fname = f"./summary/summary_{args.model}_ADC:5_Cellbit:2_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}.txt"
  with open(fname) as f:
    lines = f.readlines()
    for i, l in enumerate(lines):
