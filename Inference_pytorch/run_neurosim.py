...
def make_args(config):
  SA_ROW ,SA_COL, PE, Tile ,ADC, Cellbit= config
  return f'bash ./layer_record_{args.model}/trace_command.sh {args.model} {SA_ROW} {SA_COL} {PE} {Tile} {ADC} {Cellbit}| tee {args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{SA_ROW}_SA_col:{SA_COL}_PE:{PE}_TL:{Tile}.txt'
  ...
  with open(f"./layer_record_{args.model}/trace_command.sh") as f:
    ...
    if len(lines) == 1:
      for line in lines:
        ...
        edited_lines.append("ADC=$6\n")
        edited_lines.append("CELLBIT=$7\n")
        update = line + " $ADC $CELLBIT $MODEL $SA_ROW $SA_COL $PE $Tile"
