...
pueue add --group $MODEL_NAME "python validate_booksim_hetero.py  --model=$MODEL_NAME --latency=$LATENCY --power=$POWER --area=$AREA --heterogeneity=$HETEROGENEITY --search_accuracy=$SEARCH_ACCURACY"
...
if [ "$STRATEGY" = "constrain" ]; then
    ...
    python topsis_singlemodel.py --model=$MODEL_NAME --heterogeneity=$HETEROGENEITY --latency=$LATENCY --power=$POWER --area=$AREA --search_accuracy=$SEARCH_ACCURACY --constrain_latency=${constrain[0]} --constrain_power=${constrain[1]} --constrain_area=${constrain[2]}
else
    numbers=${STRATEGY#*[}  
    numbers=${numbers%]*}
    IFS=',' read -r -a weight <<< "$numbers"
  
    python topsis_singlemodel.py --model=$MODEL_NAME --heterogeneity=$HETEROGENEITY --latency=$LATENCY --power=$POWER --area=$AREA --search_accuracy=$SEARCH_ACCURACY --weight_latency=${weight[0]} --weight_power=${weight[1]} --weight_area=${weight[2]}
fi