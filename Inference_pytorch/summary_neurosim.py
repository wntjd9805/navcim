...
for i in CONFIG:
  ...
  fname = f"{args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}.txt"
  readLatency=[]
  power=[]
  summary=[]
    ...
    for i, l in enumerate(lines):
      ...
      if l.find("Summary") != -1:
        for k in range(len(lines)-i):
          l =lines[i+k].strip('\n')
          summary.append(l)    
  f = open(f'./summary/summary_{args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}.txt','w', newline='')
  wr = csv.writer(f)
  for c in readLatency:
    wr.writerow([c])
  for c in summary:
    wr.writerow([c])
  shutil.move(f"shape_{args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}",f"./shape/shape_{args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}")
  shutil.move(f"{args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}.txt",f"./neurosim_result/{args.model}_ADC:{ADC}_Cellbit:{Cellbit}_SA_row:{NUM_ROW}_SA_col:{NUM_COL}_PE:{PE}_TL:{Tile}.txt")
